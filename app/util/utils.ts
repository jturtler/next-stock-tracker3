...
export const cloneJSONObject = ( obj: any | any[]) => {
    return JSON.parse(JSON.stringify(obj));
}

export const isEmptyJSON = ( obj: any ): boolean => {
    return obj === null || Object.keys(obj).length === 0;
}

export const removeFromArray = function( list: any[], value: string, propertyName: string )
{
	let index: any;
...
export const sortArrayByDate  = (list: any): any[] => {
...
// Requires 'moment'
export const getDatesRangeStr = ( daysCount: number ): string => {
    return moment().add( -daysCount, 'days' ).format('YYYY-MM-DD') + ' - ' + moment().format('YYYY-MM-DD');
};
...
export const generateMonthList = (startDate: Date, endDate: Date): any[] => {
  ...
  export const generateYearList = (startDate: Date, endDate: Date): any[] => {
...
export const generateQuarterList = (startDate: Date, endDate: Date): any[] => {
    const quarters: any[] = [];
...
export const findItemFromList = ( list: any[], value: any, propertyName: string ) =>
{
...
export const convertUrlSearchParamToJson = (urlSearchParams: URLSearchParams): any => {
    const json = {} as any;
