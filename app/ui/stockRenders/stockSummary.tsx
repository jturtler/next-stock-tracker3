			...
			{ quotesData?.quotes && 
				<div>
					<div className="text-xs flex flex-col gap-1">
						<div className="bg-violet-200 rounded-sm p-1 w-fit">
							Daily split? <span>Open: {summaryData.open?.toFixed(1)}</span> , Close:  {summaryData.close?.toFixed(1)}</div>
						<div className="bg-gray-200 rounded-sm p-1 w-fit">Low: {summaryData.low?.toFixed(1)}, High: {summaryData.high?.toFixed(1)}</div>
						<div className="bg-red-200 rounded-sm p-1 w-fit">Volume High: {summaryData.volume}</div>
					</div>
					<div className='pt-2'>
